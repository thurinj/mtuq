...
def initialise_mt(depth=None, latitude=None, longitude=None):
    ...
    Mw = CMAESParameters('Mw', 4, 5, scaling='log', repair='rand_based', projection=to_rho)
    ...
    if depth:
        if not isinstance(depth, dict):
            raise TypeError('depth argument should be a dict')
        else:
            depth = CMAESParameters(**depth)
    ...
    if latitude:
        if not isinstance(latitude, dict):
            ...
            latitude = CMAESParameters(**latitude)
    ...
    if longitude:
        if not isinstance(longitude, dict):
            ...
            longitude = CMAESParameters(**longitude)
